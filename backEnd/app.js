...
var _dirname = "/Users/aarya/Documents/GitHub/tictactoi/backEnd/src/"
...
myDB (async (client)=>{
    ...
    io.on("connection", socket=>{
        ...
        if(socket.request.user.logged_in){
            socket.on("go to next level", (data)=>{
                ...
                collection.updateOne({username:data}, {$set:{gamePlayed:0, points:0}})
                loungeUserArray = []
                console.log("update for semi finals")
            })
            var date = new Date();
            console.log(tournamentTimeline.register.endTime, "endtime register", date.getTime(), "time now")
            console.log(tournamentTimeline.register.endTime> date.getTime(), "condition to start the socket connection")
        ...
        var disconnectAdjustments = (room, userOrder) => {
           ...
           if(rooms[room]){
            rooms[room]["users"].splice(userOrder, 1,{"userDetails":null, socketId: null, "userTurn":  rooms[room]["users"][userOrder]["userTurn"], "userType": rooms[room]["users"][userOrder]["userType"],"userOrder": userOrder});
            console.log(rooms[room]["usersNumber"], "number of users before disconnect")
           ...
           Object.keys(rooms).forEach(room=>{
               ...
               if(rooms[roomName]){
                   if(rooms[roomName]["usersNumber"]>0){
                    ...
                    console.log("opponent disconnect changes sent" , roomName , rooms[roomName])
                    console.log("these are the rooms after changes", rooms)
                    io.to(roomName).emit("changes on disconnect", {room:roomName, players:rooms[roomName]})
               }else if(rooms[roomName]["usersNumber"]<=0 || nullUsers(rooms[roomName]["users"])){
        ...
        socket.on("finished game", (data)=>{
            ...
            collection.updateOne(
                ...
                {$set:{gamePlayed:data.user.userDetails.gamePlayed, points:data.user.userDetails.points}},
                function(err, doc){
                    if(err){
                        console.log(err)
                    }else{
                        console.log('update tot points and gamePlayed done on finished game')
                    }
                })
        ...
        if(currentUsers <=40){
            ...
            if(arrayWaitingRooms[0]){
                ...
                rooms[arrayWaitingRooms[0]]["users"][1]["userDetails"] = socket.request.user;
                console.log(socket.request.user, "user deets when adding to room await")
                ...
                rooms[arrayWaitingRooms[0]]["users"][1]["room"] = arrayWaitingRooms[0]
                rooms[arrayWaitingRooms[0]]["usersNumber"] = 2;
                
                socket.join(arrayWaitingRooms[0])
                
                io.to(arrayWaitingRooms[0]).emit("entered room", {players:rooms[arrayWaitingRooms[0]], room:arrayWaitingRooms[0]})
                ...
                console.log(socket.request.user, "user deets when adding to new room await")
                ...
                io.to(roomName).emit("entered room", {players:rooms[roomName], room:roomName})
                roomsCounter++
    ...
    console.log(e)
      ...
      res.render(process.cwd()+'/frontEnd', { title: e, message: 'Unable to login' });
    }); 
  }); 
